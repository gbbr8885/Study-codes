...
if historico_nt is not None:
    ...
    st.markdown(f'## <span style="font-family:{fonte}, serif; color:{cor}; font-size:px">IRA individual: {irai_ultimo_semestre}</span>', unsafe_allow_html=True)

    if indices is not None:

        st.markdown(f'## <span style="font-family:{fonte}, serif; color:{cor}; font-size:px">IRA geral: com 95% de confiança, seu IRA geral está entre {irag_estimado_1} e {irag_estimado_2}. Uma boa estimativa pontual é: {irag_estimado_3}</span>', unsafe_allow_html=True)
    ...
    st.markdown(f'## <span style="font-family:{fonte}, serif; color:{cor}; font-size:px">Maior IRA: {max(irasi,irai_ultimo_semestre)}</span>', unsafe_allow_html=True)
